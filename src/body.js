  ...
  */
  sort: function (column, direction) {
    ...
    if (Backbone.PageableCollection &&
      ...
      // When sorting collection in server side mode, switch directly to first page for paged collections
      // See also: parameter goBackFirstOnSort in backgrid-paginator
      // See also: Bug #1958 in tind bugzilla
      else if (collection.mode === "server" && collection.getFirstPage) {
        collection.getFirstPage({reset: true, success: function() {
          collection.trigger("backgrid:sorted", column, direction, collection);
        }});
      }
